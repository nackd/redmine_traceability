...
issues of two trackers of a project.

== Features

* Shows a summary view of the relations between the two configured trackers
(e.g. requirements vs. test cases).
...
on (Administration -> Plugins -> Traceability Matrix -> Configure).

The plugin adds a new module (Traceability Matrix) which you can enable in the
desired projects.

There is also a new permission (View traceability matrix) which you should
grant to the user roles allowed to see the matrix.
